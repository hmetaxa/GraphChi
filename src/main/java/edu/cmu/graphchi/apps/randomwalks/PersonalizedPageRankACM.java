 ...
 * @author Aapo Kyrola
 */
public class PersonalizedPageRankACM implements WalkUpdateFunction<EmptyType, EmptyType> {
    ...
    public PersonalizedPageRankACM(String companionUrl, String baseFilename, int nShards, int firstSource, int numSources, int walksPerSource) throws Exception {
    ...
    private int execute(int numIters) throws Exception {
        File graphFile = new File(baseFilename);

        //  for (int i = 0; i < drunkardMobEngine.getEngine().numVertices(); i = i + numSources) {
        ...
        int curNumSources = firstSource + numSources > drunkardMobEngine.getEngine().numVertices() ? drunkardMobEngine.getEngine().numVertices() - firstSource : numSources;
        drunkardJob.configureSourceRangeInternalIds(firstSource, curNumSources, numWalksPerSource);
...
//                + (firstSource + numSources - 1) + ".top" + nTop, nTop);
        ...
        return curNumSources + firstSource == drunkardMobEngine.getEngine().numVertices() ? -1 : curNumSources + firstSource;
        // }
    }
        ...
        if (numOutEdges > 0) {
            for (int i = 0; i < numWalks; i++) {
                ...
                if (randomGenerator.nextDouble() < RESET_PROBABILITY) {
                    ...
                    VertexIdTranslate vertexIdTranslate = this.drunkardMobEngine.getVertexIdTranslate();
                    int initialVertex = vertexIdTranslate.backward(vertex.getId());

                    int nextHop = vertex.getOutEdgeId(randomGenerator.nextInt(numOutEdges));
                    int nextHopInitial = vertexIdTranslate.backward(nextHop);
        ...
        try {
            ...
            if (baseFilename.equals("pipein")) {     // Allow piping graph in
                ...
                if (!new File(ChiFilenames.getFilenameIntervals(baseFilename, nShards)).exists()) {
                    ...
                    // int maxVertex = sharder.hashCode();
                } else {
            ...
            int numSources = 100000;//1397238; //Integer.parseInt(cmdLine.getOptionValue("nsources"));
            ...
            while (haveMore) {
                PersonalizedPageRankACM pp = new PersonalizedPageRankACM(companionUrl, baseFilename, nShards,
                        firstSource, numSources, walksPerSource);
                firstSource = pp.execute(nIters);
                haveMore = firstSource != -1;
